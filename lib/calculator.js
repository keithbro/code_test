...
// Using big.js for precise decimal arithmetic...
...
const findApplicableRule = (rules, weekNumber) => {
  const isRuleApplicable = (rule) => {
    if (weekNumber < rule.applicableWeeks.first) return false
    if (rule.applicableWeeks.last === undefined) return true
    return weekNumber <= rule.applicableWeeks.last;
  }
...
const parsePeopleJSON = (path) => {
  /**
   * Transform values to objects at the earliest possible moment. Why?
   *
   * 1. Only need to do it once - DRY
   * 2. We don't have to remember to do it when we need it - Less error-prone
   */
...
const parseRulesJSON = (path) => {
  /**
   * Splitting in to first and last makes it more machine readable and hence
   * easier to identify the applicable rule for a given week number.
   */
